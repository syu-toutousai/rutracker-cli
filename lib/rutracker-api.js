...
RutrackerApi.prototype.login = function(username, password) {
  ...
  var req = https.request(options, function(res) {
    if (res.statusCode == '302') {
      that.cookie = res.headers['set-cookie'].map(function(cookie) {
...
RutrackerApi.prototype.search = function(_query, _callback) {
  ...
  var req = https.request(options, function(res) {
    if (res.statusCode == '200') {
      ...
      throw Error('HTTP code is ' + res.statusCode + ' redirect to: ' + res.headers.location);
    }
  });
...
RutrackerApi.prototype.parseSearch = function(rawHtml, callback) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    results.push({
      ...
      size     : formatSize( size.attr('data-ts_text') ),
      seeds    : seeds.find('b').html(),
      leechs   : leechs.find('b').html(),
      url      : 'https://' + this.host + '/forum/' + title.find('div a').attr('href')
    });
